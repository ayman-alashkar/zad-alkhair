...
function quranPage(surahs,juz){
  ...
  return `${head({title,description,canonical,image:"/social/reader-general.jpg",jsonLd:schema})}<body>${siteHead}${crumbs([["زاد الخير","/"],["القرآن العظيم"]])}<main><h1>القرآن العظيم في زاد الخير</h1><p class="lead">اقرأ القرآن العظيم في مصحف صُمّم للعناية بصفحة القراءة، مع التفسير الوجيز للدكتور وهبة الزحيلي والاستماع إلى تلاوات نخبة من القراء.</p><div class="actions"><a class="btn primary" href="/reader.html?home=1&amp;from=zad">فتح قارئ القرآن</a><a class="btn secondary" href="/install/">تثبيت زاد الخير</a></div><h2>الاستماع إلى القرآن بصوت القراء</h2><div class="reciter-grid">${reciterCards}</div><h2 id="surahs">سور القرآن الكريم</h2><p>اختر السورة لقراءتها، والاستماع إليها، والرجوع إلى تفسير آياتها.</p><nav class="link-grid" aria-label="فهرس سور القرآن">${surahLinks}</nav><h2 id="ajzaa">أجزاء القرآن الكريم</h2><nav class="link-grid" aria-label="فهرس أجزاء القرآن">${juzLinks}</nav><h2>القراءة والاستماع دون اتصال</h2><p>يمكن تجهيز القرآن والتفسير كاملين للعمل دون اتصال، ثم تنزيل التلاوات التي تختارها سورةً سورةً للاستماع إليها لاحقًا.</p></main>${footer}</body></html>`;
}
function reciterPage(r,surahs){
  ...
  return `${head({title,description,canonical,image:"/social/reader-general.jpg",jsonLd:schema})}<body>${siteHead}${crumbs([["زاد الخير","/"],["القرآن العظيم","/quran/"],[r.display]])}<main><div class="eyebrow">تلاوات القرآن الكريم</div><h1>القرآن بصوت ${esc(r.display)}</h1><p class="lead">استمع إلى تلاوة سور القرآن بصوت ${esc(r.display)}${r.extra?` — ${esc(r.extra)}`:""}، واقرأ الآيات من مصحف زاد الخير مع التفسير الوجيز للدكتور وهبة الزحيلي.</p><p class="note">اختر سورة من القائمة. سيفتح القارئ على بدايتها ويختار صوت ${esc(r.name)} تلقائيًا.</p><nav class="link-grid" aria-label="سور القرآن بصوت ${esc(r.name)}">${links}</nav><div class="actions"><a class="btn secondary" href="/quran/">العودة إلى فهرس القرآن</a></div></main>${footer}</body></html>`;
}
